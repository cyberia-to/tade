...
TADE reserves a small namespace for stream-level control: frames that
manage the stream itself, independent of any dialect. Every TADE consumer
MUST understand the reserved set even if it does not understand the
declared dialect.

## Reserved type byte

TADE reserves the type byte `*` (0x2A) for stream control. No dialect
may use `*` as a type byte. All `(*, *)` pairs are owned by TADE.

This keeps dialect vocabulary and stream control fully separated: a
consumer encountering `*` knows the frame is TADE-defined, regardless of
which dialect is active.

## Reserved frames

The minimum set in this version of TADE:
...
These three are the entire TADE Layer 1 vocabulary.

## `(*, k)` — Dialect declaration

The first frame of a TADE stream SHOULD be a dialect declaration. Its
...
Either side of a TADE exchange may send `(*, c)` to signal abort. The
...
Stream control is what every TADE consumer needs regardless of dialect:
...
TADE. A consumer can implement TADE in a few hundred lines and handle
arbitrary streams; semantic richness comes from whichever dialect the
stream declares.

## Future reservations

TADE may extend the `(*, *)` namespace in future revisions. The
